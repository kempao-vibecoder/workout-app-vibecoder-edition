...
export default function DashboardPage() {
  ...
  async function loadDashboardData() {
    ...
    const { data: logsData } = await supabase
      ...
      `)
      .order("date", { ascending: false }) // Ordena pela data do treino
      .order("created_at", { ascending: false }) // Desempate pelo horário de criação
      .limit(30)
  ...
  // Procura se existe algum treino que tenha o dia de hoje na lista (pega só a primeira palavra ex: "segunda")
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[10px] text-muted-foreground capitalize flex gap-2 items-center">
                                            <span>{new Date(log.date).toLocaleDateString("pt-BR", { weekday: 'short', day: '2-digit', month: 'short' })}</span>
                                            
                                            {/* EXIBIÇÃO DO HORÁRIO AQUI */}
                                            {log.created_at && (
                                              <>
                      ...
                      <span className="flex items-center gap-2">
                        ...
                        {selectedLog.created_at && (
                          <>
                             ...
                             {new Date(selectedLog.created_at).toLocaleTimeString("pt-BR", { hour: '2-digit', minute: '2-digit' })}
                          </>
                        )}
                      </span>
                    )}
